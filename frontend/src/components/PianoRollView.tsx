...
export default function PianoRollView({ song, audioInitialized, onUpdateSong, onEnsureAudioInit }: PianoRollViewProps) {
    ...
    const [hoveredNote, setHoveredNote] = useState<{ trackId: string; note: Note } | null>(null);
        ...
        <div className="h-full w-full bg-zinc-950 overflow-hidden">
            <div className="h-full flex flex-col">
                ...
                <div className="px-6 py-4 border-b border-zinc-800 flex-shrink-0">
                    ...
                    <p className="text-sm text-zinc-500 mt-1">Click on a track to expand • Click notes to preview</p>
                </div>
                                        ...
                                        <div className="w-full h-12 bg-zinc-900/50 rounded-lg relative overflow-hidden">
                                            {track.notes.map((note, noteIndex) => (
                                                <div
                                                    ...
                                                    style={{
                                                        left: `${(noteIndex / Math.max(track.notes.length - 1, 1)) * 90}%`,
                                                        width: '8%',
                                                        top: '40%'
                                                    }}
                                                    onClick={(e) => {
                                                    ...
                                                    title={`${note.pitch} at ${note.start_time}`}
                                                />
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    velocity: 80
                                                ...
                                                <div className="flex-1 relative">
                                                    ...
                                                    {track.notes.map((note, noteIndex) => (
                                                        <div
                                                            key={noteIndex}
                                                            className={`absolute ${trackColor} rounded cursor-pointer hover:opacity-80 transition-opacity shadow-sm`}
                                                            style={{
                                                            ...
                                                            onClick={() => playNote(track.id, note)}
                                                            onMouseEnter={() => setHoveredNote({ trackId: track.id, note })}
                                                            onMouseLeave={() => setHoveredNote(null)}
                                                            title={`${note.pitch} • ${note.start_time} • ${note.duration}`}
                                                        />
                                                    ))}
                                                </div>
                                            </div>
                                            ...
                                            {hoveredNote && hoveredNote.trackId === track.id && (
                                                <div className="absolute top-2 right-2 bg-zinc-800 border border-zinc-700 rounded px-2 py-1 text-xs text-zinc-300">
                                                    {hoveredNote.note.pitch} • {hoveredNote.note.start_time}
                                                </div>
                                            )}
                                        </div>
                                    </div>
